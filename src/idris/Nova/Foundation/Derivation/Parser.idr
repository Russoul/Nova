...
parseTypingRule =
  ...
  (do str_ "ty-pi-cong"; space; ctx <- parseCtx; sp; str_ "⊦"; sp
      ty0 <- parseTy; sp; str_ "≐"; sp; ty1 <- parseTy
      case (ty0, ty1) of
        (PiTy a0 b0, PiTy a1 b1) => pure (TyEqCongPi ctx a0 b0 a1 b1)
        _ => fail "ty-pi-cong: expected (A₀ → B₀) ≐ (A₁ → B₁)") <|>
  (do str_ "ty-sigma-cong"; space; ctx <- parseCtx; sp; str_ "⊦"; sp
      ty0 <- parseTy; sp; str_ "≐"; sp; ty1 <- parseTy
      case (ty0, ty1) of
        (SigmaTy a0 b0, SigmaTy a1 b1) => pure (TyEqCongSigma ctx a0 b0 a1 b1)
        _ => fail "ty-sigma-cong: expected (A₀ ⨯ B₀) ≐ (A₁ ⨯ B₁)") <|>
  (do str_ "ty-quotient-cong"; space; ctx <- parseCtx; sp; str_ "⊦"; sp
      ty0 <- parseTy; sp; str_ "≐"; sp; ty1 <- parseTy
      case (ty0, ty1) of
  ...
  (do str_ "el-pi-e"; space; ctx <- parseCtx; sp; str_ "⊦"; sp; e <- parseElem
      case e of
        PiApp f arg => pure (ElemWfPiAppInfer ctx f arg)
        _           => fail "el-pi-e: expected (f : A → B) e or f e") <|>
