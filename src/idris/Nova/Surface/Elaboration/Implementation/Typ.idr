...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (PiTy r x dom cod) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (ImplicitPiTy r x dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
  (omega, cod') <- liftUnifyM $ newTypeMeta omega (ctx :< (x, OmegaVarElim dom' Id)) SolveByElaboration
  let omega = Typ.instantiateByElaboration omega meta (ImplicitPiTy x (OmegaVarElim dom' Id) (OmegaVarElim cod' Id))
  return (Success omega [TypeElaboration ctx dom dom', TypeElaboration (ctx :< (x, OmegaVarElim dom' Id)) cod cod'])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (SigmaTy r x dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
  (omega, cod') <- liftUnifyM $ newTypeMeta omega (ctx :< (x, OmegaVarElim dom' Id)) SolveByElaboration
  let omega = Typ.instantiateByElaboration omega meta (SigmaTy x (OmegaVarElim dom' Id) (OmegaVarElim cod' Id))
  return (Success omega [TypeElaboration ctx dom dom', TypeElaboration (ctx :< (x, OmegaVarElim dom' Id)) cod cod'])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (FunTy r dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (ProdTy r dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
  (omega, cod') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
  let omega = Typ.instantiateByElaboration omega meta (SigmaTy "_" (OmegaVarElim dom' Id) (OmegaVarElim cod' Wk))
  return (Success omega [TypeElaboration ctx dom dom', TypeElaboration ctx cod cod'])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (TyEqTy r a b) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (ElEqTy r a b t) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (SigmaVal r a b) meta =
  return (Error "(_, _) is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx tm@(App r (Var r0 x) es) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (OneVal x) _) meta = M.do
  return (Error "() is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (NatVal0 x) _) meta = M.do
  return (Error "Z is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (NatVal1 _) _) meta = M.do
  return (Error "S is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx tm@(App r (NatElim _) _) meta = M.do
  (omega, t') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
  let omega = Typ.instantiateByElaboration omega meta (El (OmegaVarElim t' Id))
  return (Success omega [ElemElaboration ctx tm t' UniverseTy])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx tm@(App r (ZeroElim _) _) meta = M.do
  (omega, t') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
  let omega = Typ.instantiateByElaboration omega meta (El (OmegaVarElim t' Id))
  return (Success omega [ElemElaboration ctx tm t' UniverseTy])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App _ (EqElim _) _) meta = M.do
  return (Error "J is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (EqVal x) _) meta = M.do
  return (Error "Refl is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (NatTy x) []) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (ZeroTy x) []) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (OneTy x) []) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (UniverseTy x) []) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (UnnamedHole r0 Nothing) []) meta = M.do
  ...
  return (Success omega [])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (UnnamedHole r0 (Just vars)) []) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (Hole r0 n (Just vars)) es) meta =
  case (lookup n omega) of
    ...
    Nothing => M.do
      ...
      let omega = Typ.instantiateByElaboration omega meta (OmegaVarElim n (WkN (length ctx `minus` length regctxPrefix)))
      return (Success omega [])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (Unfold r0 x) _) meta = M.do
  return (Error "! is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx tm@(App r (PiBeta r0) _) meta = M.do
  return (Error "Π-β is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx tm@(App r (SigmaBeta1 r0) _) meta = M.do
  return (Error "Σ-β₁ is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx tm@(App r (SigmaBeta2 r0) _) meta = M.do
  return (Error "Σ-β₂ is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (PiEta r0) _) meta = M.do
  return (Error "Π-η is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (SigmaEta r0) _) meta = M.do
  return (Error "Σ-η is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (NatBetaZ r0) _) meta = M.do
  return (Error "ℕ-β-Z is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (NatBetaS r0) _) meta = M.do
  return (Error "ℕ-β-S is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (PiEq r0) _) meta = M.do
  return (Error "Π⁼ is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (SigmaEq r0) _) meta = M.do
  return (Error "Σ⁼ is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (OneEq r0) _) meta = M.do
  return (Error "𝟙⁼ is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (Tm _ tm) []) meta = M.do
  return (Success omega [TypeElaboration ctx tm meta])
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (Tm _ tm) es) meta = M.do
  return (Error "_ _ is not a type")
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (Tac r alpha) meta = M.do
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (El r0) [(_, Arg ([], el))]) meta = M.do
  -- ⟦e⟧ : 𝕌
  -- -------------------
  -- ⟦El e⟧ ⇝ El e' type
...
Nova.Surface.Elaboration.Interface.elabType ops sig omega ctx (App r (El r0) _) meta = M.do
