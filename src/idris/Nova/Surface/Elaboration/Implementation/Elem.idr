...
elabElemNuOtherwise ops sig omega ctx tm@(PiTy r x dom cod) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx tm@(ImplicitPiTy r x dom cod) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty UniverseTy)
  return (Success omega [ElemElaboration ctx tm meta UniverseTy])
elabElemNuOtherwise ops sig omega ctx tm@(SigmaTy r x dom cod) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty UniverseTy)
  return (Success omega [ElemElaboration ctx tm meta UniverseTy])
elabElemNuOtherwise ops sig omega ctx tm@(FunTy x y z) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty UniverseTy)
  return (Success omega [ElemElaboration ctx tm meta UniverseTy])
elabElemNuOtherwise ops sig omega ctx tm@(ProdTy x y z) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty UniverseTy)
  return (Success omega [ElemElaboration ctx tm meta UniverseTy])
elabElemNuOtherwise ops sig omega ctx tm@(TyEqTy r a b) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty UniverseTy)
  return (Success omega [ElemElaboration ctx tm meta UniverseTy])
elabElemNuOtherwise ops sig omega ctx tm@(ElEqTy r a b t) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty UniverseTy)
  return (Success omega [ElemElaboration ctx tm meta UniverseTy])
elabElemNuOtherwise ops sig omega ctx tm@(PiVal r x f) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx tm@(ImplicitPiVal r x f) meta ty = M.do
  (omega, dom) <- liftUnifyM $ newTypeMeta omega ctx SolveByUnification
  (omega, cod) <- liftUnifyM $ newTypeMeta omega (ctx :< (x, OmegaVarElim dom Id)) SolveByUnification
...
elabElemNuOtherwise ops sig omega ctx tm@(SigmaVal r a b) meta ty = M.do
  (omega, dom) <- liftUnifyM $ newTypeMeta omega ctx SolveByUnification
...
elabElemNuOtherwise ops sig omega ctx (App r (Var r0 x) es) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx tm@(App r (NatVal0 x) []) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx tm@(App r (OneVal x) []) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx tm@(App r (NatVal1 _) [t]) meta ty = M.do
  omega <- liftUnifyM $ addConstraint omega (TypeConstraint ctx ty NatTy)
  return (Success omega [ElemElaboration ctx tm meta NatTy])
elabElemNuOtherwise ops sig omega ctx (App r (NatVal1 x) _) meta ty =
  return (Error "S applied to a wrong number of arguments")
elabElemNuOtherwise ops sig omega ctx (App r (NatElim _) ((_, Arg ([x], schema)) :: (_, Arg ([], z)) :: (_, Arg ([y, h], s)) :: (_, Arg ([], t)) :: es)) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App r (ZeroElim _) ((_, Arg ([], t)) :: es)) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App _ (EqElim _) ((_, Arg ([], elemTy)) :: (_, Arg ([], a0)) :: (_, Arg ([x, h], schema)) :: (_, Arg ([], r)) :: (_, Arg ([], a1)) :: (_, Arg ([], e)) :: es)) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx tm@(App r (EqVal x) []) meta ty = M.do
  -- We can have one of:
  --   ⟦Refl⟧ : (? ≡ ? ∈ ?)
  --   ⟦Refl⟧ : (? ≡ ? type)
  -- Can't decide yet
...
elabElemNuOtherwise ops sig omega ctx (App r (UnnamedHole r0 Nothing) []) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App r (UnnamedHole r0 (Just vars)) []) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App r (Hole r0 n Nothing) es) meta ty = M.do
  case (lookup n omega) of
    ...
    Nothing => M.do
      ...
      return (Success omega [])
elabElemNuOtherwise ops sig omega ctx (App r (Hole r0 n (Just vars)) es) meta ty = M.do
  case (lookup n omega) of
    Just _ => return (Error "Hole already exists: \{n}")
    Nothing => M.do
      let Just regctxPrefix = pickPrefix (cast ctx) vars
      ...
      let omega = Elem.instantiateByElaboration omega meta (OmegaVarElim n (WkN (length ctx `minus` length regctxPrefix)))
      return (Success omega [])
elabElemNuOtherwise ops sig omega ctx (App r (Unfold r0 x) []) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App r (Unfold r0 x) _) meta ty = M.do
  return (Error "!\{x} applied to a wrong number of arguments")
-- Π-β A (x. B) (x. f) e : (x ↦ f) e ≡ f[e/x] ∈ B[e/x]
elabElemNuOtherwise ops sig omega ctx (App r (PiBeta r0) [(_, Arg ([], dom)), (_, Arg ([x'], cod)), (_, Arg ([x], f)), (_, Arg ([], e))]) meta ty = M.do
...
-- Π-η f : (x ↦ f x) ≡ f ∈ (x : A) → B
elabElemNuOtherwise ops sig omega ctx (App r (PiEta r0) [(_, Arg ([], f))]) meta ty = M.do
  (omega, dom) <- liftUnifyM $ newTypeMeta omega ctx SolveByUnification
  (omega, cod) <- liftUnifyM $ newTypeMeta omega (ctx :< ("_", OmegaVarElim dom Id)) SolveByUnification
...
elabElemNuOtherwise ops sig omega ctx (App r (SigmaBeta1 r0) [(_, Arg ([], dom)), (_, Arg ([x], cod)), (_, Arg ([], a)), (_, Arg ([], b))]) meta ty = M.do
  (omega, dom') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
  (omega, cod') <- liftUnifyM $ newTypeMeta omega (ctx :< (x, OmegaVarElim dom' Id)) SolveByElaboration
...
elabElemNuOtherwise ops sig omega ctx (App r (SigmaBeta2 r0) [(_, Arg ([], dom)), (_, Arg ([x], cod)), (_, Arg ([], a)), (_, Arg ([], b))]) meta ty = M.do
  (omega, dom') <- liftUnifyM $ newTypeMeta omega ctx SolveByElaboration
  (omega, cod') <- liftUnifyM $ newTypeMeta omega (ctx :< (x, OmegaVarElim dom' Id)) SolveByElaboration
  (omega, a') <- liftUnifyM $ newElemMeta omega ctx (OmegaVarElim dom' Id) SolveByElaboration
  (omega, b') <- liftUnifyM $ newElemMeta omega ctx (OmegaVarElim cod' (Ext Id (OmegaVarElim a' Id))) SolveByElaboration
...
elabElemNuOtherwise ops sig omega ctx (App r (SigmaEta r0) [(_, Arg ([], p))]) meta ty = M.do
  (omega, dom) <- liftUnifyM $ newTypeMeta omega ctx SolveByUnification
  (omega, cod) <- liftUnifyM $ newTypeMeta omega (ctx :< ("_", OmegaVarElim dom Id)) SolveByUnification
...
elabElemNuOtherwise ops sig omega ctx (App r (NatBetaZ r0) ((_, Arg ([x], schema)) :: (_, Arg ([], z)) :: (_, Arg ([y, h], s)) :: [])) meta ty = M.do
  (omega, schema') <- liftUnifyM $ newTypeMeta omega (ctx :< (x, NatTy)) SolveByElaboration
  (omega, z') <- liftUnifyM $ newElemMeta omega ctx (ContextSubstElim (OmegaVarElim schema' Id) (Ext Id NatVal0)) SolveByElaboration
  (omega, s') <- liftUnifyM $ newElemMeta omega (ctx :< (y, NatTy) :< (h, OmegaVarElim schema' Id))
                                  (ContextSubstElim (OmegaVarElim schema' Id) (Ext (WkN 2) (NatVal1 (CtxVarN 1)))) SolveByElaboration
...
elabElemNuOtherwise ops sig omega ctx (App r (NatBetaS r0) [(_, Arg ([x], schema)), (_, Arg ([], z)), (_, Arg ([y, h], s)), (_, Arg ([], t))]) meta ty = M.do
  (omega, schema') <- liftUnifyM $ newTypeMeta omega (ctx :< (x, NatTy)) SolveByElaboration
  (omega, z') <- liftUnifyM $ newElemMeta omega ctx (ContextSubstElim (OmegaVarElim schema' Id) (Ext Id NatVal0)) SolveByElaboration
  (omega, s') <- liftUnifyM $ newElemMeta omega ((ctx :< (y, NatTy)) :< (h, OmegaVarElim schema' Id))
                                  (ContextSubstElim (OmegaVarElim schema' Id) (Ext (WkN 2) (NatVal1 (CtxVarN 1)))) SolveByElaboration
...
-- 𝟙⁼ a b : a ≡ b ∈ 𝟙
elabElemNuOtherwise ops sig omega ctx (App r (OneEq r0) [(_, Arg ([], a)), (_, Arg ([], b))]) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App r (PiEq r0) [(_, Arg ([x], f)), (_, Arg ([y], g)), (_, Arg ([z], p))]) meta ty = M.do
  (omega, dom) <- liftUnifyM $ newTypeMeta omega ctx SolveByUnification
...
elabElemNuOtherwise ops sig omega ctx (App r (SigmaEq r0) [(_, Arg ([], a0)), (_, Arg ([], b0)), (_, Arg ([], a1)), (_, Arg ([], b1)), (_, Arg ([], a)), (_, Arg ([], b))]) meta ty = M.do
  (omega, dom) <- liftUnifyM $ newTypeMeta omega ctx SolveByUnification
  (omega, cod) <- liftUnifyM $ newTypeMeta omega (ctx :< ("_", OmegaVarElim dom Id)) SolveByUnification
...
elabElemNuOtherwise ops sig omega ctx (App r (Tm _ tm) []) meta ty = M.do
  return (Success omega [ElemElaboration ctx tm meta ty])
elabElemNuOtherwise ops sig omega ctx (App r (Tm _ tm) (_ :: _)) meta ty = M.do
...
-- Ξ Ω ⊦ ⟦α⟧ ⇝ α' : ε ⇒ ε (Γ ⊦ A)
-- Ξ Ω Γ ⊦ ⟦tac α⟧ ⇝ α' · : A
elabElemNuOtherwise ops sig omega ctx (Tac r alpha) meta ty = M.do
...
elabElemNuOtherwise ops sig omega ctx (App r (El r0) _) meta ty = M.do
...
elabElemNuAtUniverse : Params
                    ...
                    -> OmegaName
                    -> ElabM Elaboration.Result
elabElemNuAtUniverse ops sig omega ctx (PiTy r x dom cod) meta = M.do
...
elabElemNuAtUniverse ops sig omega ctx (ImplicitPiTy r x dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
  (omega, cod') <- liftUnifyM $ newElemMeta omega (ctx :< (x, El $ OmegaVarElim dom' Id)) UniverseTy SolveByElaboration
  let omega = Elem.instantiateByElaboration omega meta (ImplicitPiTy x (OmegaVarElim dom' Id) (OmegaVarElim cod' Id))
  return (Success omega [ElemElaboration ctx dom dom' UniverseTy, ElemElaboration (ctx :< (x, El $ OmegaVarElim dom' Id)) cod cod' UniverseTy])
elabElemNuAtUniverse ops sig omega ctx (SigmaTy r x dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
  (omega, cod') <- liftUnifyM $ newElemMeta omega (ctx :< (x, El $ OmegaVarElim dom' Id)) UniverseTy SolveByElaboration
  let omega = Elem.instantiateByElaboration omega meta (SigmaTy x (OmegaVarElim dom' Id) (OmegaVarElim cod' Id))
  return (Success omega [ElemElaboration ctx dom dom' UniverseTy, ElemElaboration (ctx :< (x, El $ OmegaVarElim dom' Id)) cod cod' UniverseTy])
elabElemNuAtUniverse ops sig omega ctx (FunTy r dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
...
elabElemNuAtUniverse ops sig omega ctx (ProdTy r dom cod) meta = M.do
  (omega, dom') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
  (omega, cod') <- liftUnifyM $ newElemMeta omega ctx UniverseTy SolveByElaboration
  let omega = Elem.instantiateByElaboration omega meta (SigmaTy "_" (OmegaVarElim dom' Id) ((OmegaVarElim cod' Wk)))
  return (Success omega [ElemElaboration ctx dom dom' UniverseTy, ElemElaboration ctx cod cod' UniverseTy])
elabElemNuAtUniverse ops sig omega ctx (TyEqTy r a b) meta = M.do
...
elabElemNuAtUniverse ops sig omega ctx (ElEqTy r a b t) meta = M.do
...
elabElemNuAtUniverse ops sig omega ctx (App r (NatTy x) []) meta = M.do
  let omega = Elem.instantiateByElaboration omega meta NatTy
  return (Success omega [])
elabElemNuAtUniverse ops sig omega ctx (App r (ZeroTy x) []) meta = M.do
  let omega = Elem.instantiateByElaboration omega meta ZeroTy
  return (Success omega [])
elabElemNuAtUniverse ops sig omega ctx (App r (OneTy x) []) meta = M.do
...
elabElemNuAtPi : Params
              ...
              -> Typ
              -> ElabM Elaboration.Result
elabElemNuAtPi ops sig omega ctx (PiVal r x f) meta y dom cod = M.do
...
elabElemNuAtPi ops sig omega ctx tm meta y dom cod = M.do
...
elabElemNuAtSigma : Params
                 ...
                 -> Typ
                 -> ElabM Elaboration.Result
elabElemNuAtSigma ops sig omega ctx (SigmaVal r a b) meta x dom cod = M.do
...
elabElemNuAtSigma ops sig omega ctx tm meta x dom cod = M.do
...
elabElemNuAtImplicitPi : Params
                      ...
                      -> Typ
                      -> ElabM Elaboration.Result
elabElemNuAtImplicitPi ops sig omega ctx (ImplicitPiVal r x f) meta y dom cod = M.do
  (omega, f') <- liftUnifyM $ newElemMeta omega (ctx :< (x, dom)) cod SolveByElaboration
  let omega = Elem.instantiateByElaboration omega meta (ImplicitPiVal x dom cod (OmegaVarElim f' Id))
  return (Success omega [ElemElaboration (ctx :< (x, dom)) f f' cod])
elabElemNuAtImplicitPi ops sig omega ctx tm meta y dom cod = M.do
...
elabElemNuAtNat : Params
               ...
               -> OmegaName
               -> ElabM Elaboration.Result
elabElemNuAtNat ops sig omega ctx (App r (NatVal0 x) []) meta = M.do
  let omega = Elem.instantiateByElaboration omega meta NatVal0
  return (Success omega [])
elabElemNuAtNat ops sig omega ctx (App r (NatVal1 _) [(_, Arg ([], t))]) meta = M.do
  (omega, t') <- liftUnifyM $ newElemMeta omega ctx NatTy SolveByElaboration
...
elabElemNuAtNat ops sig omega ctx tm meta = M.do
...
elabElemNuAtOne : Params
               ...
               -> OmegaName
               -> ElabM Elaboration.Result
elabElemNuAtOne ops sig omega ctx (App r (OneVal x) []) meta = M.do
  let omega = Elem.instantiateByElaboration omega meta OneVal
  return (Success omega [])
elabElemNuAtOne ops sig omega ctx tm meta = M.do
...
elabElemNuAtElEq : Params
                ...
                -> Typ
                -> ElabM Elaboration.Result
elabElemNuAtElEq ops sig omega ctx (App r (EqVal x) []) meta a b t = M.do
  ...
  return (Success omega [])
elabElemNuAtElEq ops sig omega ctx tm meta a b t = M.do
...
Nova.Surface.Elaboration.Interface.elabElem ops sig omega ctx tm p ty =
  elabElemNu ops sig omega ctx tm p !(Elab.liftM $ openEval sig omega ty)
...
elabElemElimNu : Params
              ...
              -> CoreTyp
              -> ElabM Elaboration.Result
elabElemElimNu ops sig omega ctx head (ImplicitPiTy x dom cod) ((_, ImplicitArg e) :: es) meta ty = M.do
...
elabElemElimNu ops sig omega ctx head (ImplicitPiTy x dom cod) es meta ty = M.do
...
elabElemElimNu ops sig omega ctx head headTy [] meta ty = M.do
...
elabElemElimNu ops sig omega ctx head (PiTy x dom cod) ((_, Arg ([], e)) :: es) meta ty = M.do
  (omega, e') <- liftUnifyM $ newElemMeta omega ctx dom SolveByElaboration
  return (Success omega [ElemElaboration ctx e e' dom,
                         ElemElimElaboration ctx (PiElim head x dom cod (OmegaVarElim e' Id)) (ContextSubstElim cod (Ext Id (OmegaVarElim e' Id))) es meta ty])
elabElemElimNu ops sig omega ctx head (SigmaTy x dom cod) ((_, Pi1) :: es) meta ty = M.do
  return (Success omega [ElemElimElaboration ctx (SigmaElim1 head x dom cod) dom es meta ty])
elabElemElimNu ops sig omega ctx head (SigmaTy x dom cod) ((_, Pi2) :: es) meta ty = M.do
...
Nova.Surface.Elaboration.Interface.elabElemElim ops sig omega ctx head headTy es p ty =
  elabElemElimNu ops sig omega ctx head !(Elab.liftM $ openEval sig omega headTy) es p !(Elab.liftM $ openEval sig omega ty)
