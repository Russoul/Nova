...
mutual
  ...
  evalNu (PiElim f x a b e) = FailSt.do
    PiVal _ _ _ f <- eval f
      | _ => throw "eval(PiElim)"
    eval (ContextSubstElim f (Ext (Terminal Empty) a e))
  ...
  evalNu (NatElim x schema z y h s t) = FailSt.do
    ...
    case t of
      NatVal0 => eval z
      NatVal1 t => eval (ContextSubstElim s (Ext (Ext (Terminal Empty) NatTy t) (ContextSubstElim schema (Ext (Id Empty) NatTy t)) (NatElim x schema z y h s t)))
  ...
  evalNu (EqVal a ty) = return (EqVal a ty)
